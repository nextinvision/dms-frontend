...
export const AppointmentForm = ({
  ...
  // Track blob URLs to prevent them from being garbage collected
  ...
  useEffect(() => {
    if (initialData) {
      setFormData((prev) => {
        ...
        // Ensure vehicle is set if provided in initialData
        if (initialData.vehicle && initialData.vehicle.trim()) {
          updated.vehicle = initialData.vehicle;
        }
        // Ensure vehicleId is set if provided
        if (initialData.vehicleId) {
          updated.vehicleId = initialData.vehicleId;
        }
        return updated;
      });
  ...
  // Auto-populate service center if not set and user has one assigned
  useEffect(() => {
    if (!formData.serviceCenterName && !formData.serviceCenterId && customerInfo) {
      // Try to get service center from customer's service center context or nearest service center
      const { getServiceCenterContext } = require("@/shared/lib/serviceCenter");
      const context = getServiceCenterContext();

      if (context.serviceCenterId && context.serviceCenterName) {
        setFormData((prev) => ({
          ...prev,
          serviceCenterName: context.serviceCenterName || prev.serviceCenterName,
          serviceCenterId: typeof context.serviceCenterId === 'number'
            ? context.serviceCenterId.toString()
            : context.serviceCenterId || prev.serviceCenterId,
        }));
      } else if (availableServiceCenters.length > 0) {
        // Fallback to first available service center
        const firstCenter = availableServiceCenters[0];
        setFormData((prev) => ({
          ...prev,
          serviceCenterName: firstCenter.name,
          serviceCenterId: firstCenter.id,
        }));
      }
    }
  }, [formData.serviceCenterName, formData.serviceCenterId, customerInfo, availableServiceCenters]);
  ...
  // Auto-populate vehicle from vehicleInfo prop if provided and formData.vehicle is empty
  useEffect(() => {
    if (vehicleInfo) {
      const vehicleString = formatVehicleString(vehicleInfo);
      // Only update if vehicle is empty or doesn't match
      setFormData((prev) => {
        if (!prev.vehicle || prev.vehicle.trim() === "" || prev.vehicle !== vehicleString) {
          return {
            ...prev,
            vehicle: vehicleString,
            vehicleId: vehicleInfo.id || prev.vehicleId || "",
            ...mapVehicleToFormData(vehicleInfo), // Also populate vehicle details
          };
        }
        return prev;
      });
  ...
  const handleDocumentUpload = useCallback(
    async (field: "customerIdProof" | "vehicleRCCopy" | "warrantyCardServiceBook" | "photosVideos", files: FileList | null) => {
      ...
      const newFilesData: { file: File; category: FileCategory; metadata: DocumentMetadata }[] = fileArray.map(file => {
        ...
        // Track this blob URL so it doesn't get garbage collected
    ...
    async (field: "customerIdProof" | "vehicleRCCopy" | "warrantyCardServiceBook" | "photosVideos", index: number) => {
      ...
      if (metadata[index]?.url) {
        try {
          ...
          blobUrlsRef.current.delete(metadata[index].url);
        } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {mode === "edit" && (formData.updatedBy || formData.updatedAt) && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="flex items-center justify-between text-xs text-gray-600">
            {formData.updatedBy && (
              <div>
                <span className="font-semibold">Last Updated By:</span> {formData.updatedBy}
              </div>
            )}
            {formData.updatedAt && (
              <div>
                <span className="font-semibold">Last Updated:</span> {new Date(formData.updatedAt).toLocaleString()}
              </div>
            )}
          </div>
        </div>
